...
mod ast_tests {
    use kern_parser::ast_nodes::{
        Program, Declaration, Entity, Rule, Flow, Constraint, 
    ...
    fn test_rule_node_creation() {
        ...
        let rule = Rule {
            ...
            condition: Some(Expression::BinaryOp {
                left: Box::new(Expression::Identifier(Identifier { name: "user".to_string(), field: Some("age".to_string()) })),
    ...
    fn test_constraint_node_creation() {
        let constraint = Constraint {
            ...
            condition: Some(Expression::BinaryOp {
                left: Box::new(Expression::Identifier(Identifier { name: "user".to_string(), field: Some("email".to_string()) })),
    ...
    fn test_binary_operation_ast_node() {
        let expr = Expression::BinaryOp {
            left: Box::new(Expression::Identifier(Identifier { name: "a".to_string(), field: None })),
            operator: BinaryOp::Equals,
            right: Box::new(Expression::Identifier(Identifier { name: "b".to_string(), field: None })),
        };
    ...
    fn test_identifier_ast_node() {
        let id_with_field = Identifier { name: "user".to_string(), field: Some("name".to_string()) };
        let id_without_field = Identifier { name: "value".to_string(), field: None };
    ...
    fn test_nested_expressions() {
        let nested_expr = Expression::BinaryOp {
            ...
            operator: BinaryOp::Equals,
            right: Box::new(Expression::Identifier(Identifier { name: "c".to_string(), field: None })),
        };
        ...
        match nested_expr {
            Expression::BinaryOp { left, operator, right } => {
                ...
                match left.as_ref() {
                    Expression::BinaryOp { left: inner_left, operator: inner_op, right: inner_right } => {
