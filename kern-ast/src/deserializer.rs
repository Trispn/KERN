...
use crate::{
    ...
    RuleNode, SourceLocation, TypeNode, UnaryExprNode, UnaryOperator,
};
...
fn deserialize_program_node<R: Read>(reader: &mut R) -> IoResult<ProgramNode> {
    ...
    if field_count != 4 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for Program node",
        ));
    }

    // Deserialize entities
    let entities = deserialize_entity_list(reader)?;

    // Deserialize rules
    let rules = deserialize_rule_list(reader)?;

    // Deserialize flows
    let flows = deserialize_flow_list(reader)?;

    // Deserialize constraints
    let constraints = deserialize_constraint_list(reader)?;
...
fn deserialize_entity_list<R: Read>(reader: &mut R) -> IoResult<Vec<EntityNode>> {
    // Read field kind (should be NodeList)
    let mut field_kind_byte = [0u8; 1];
    reader.read_exact(&mut field_kind_byte)?;

    if field_kind_byte[0] != 1 {
        // FieldKind::NodeList as u8
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Expected NodeList field",
        ));
    }
...
fn deserialize_rule_list<R: Read>(reader: &mut R) -> IoResult<Vec<RuleNode>> {
    // Read field kind (should be NodeList)
    let mut field_kind_byte = [0u8; 1];
    reader.read_exact(&mut field_kind_byte)?;
    ...
    for _ in 0..count {
        rules.push(deserialize_rule_node(reader)?);
    }

    Ok(rules)
}

fn deserialize_flow_list<R: Read>(reader: &mut R) -> IoResult<Vec<FlowNode>> {
    // Read field kind (should be NodeList)
    let mut field_kind_byte = [0u8; 1];
    reader.read_exact(&mut field_kind_byte)?;
    ...
    for _ in 0..count {
        flows.push(deserialize_flow_node(reader)?);
    }

    Ok(flows)
}

fn deserialize_constraint_list<R: Read>(reader: &mut R) -> IoResult<Vec<ConstraintNode>> {
    // Read field kind (should be NodeList)
    let mut field_kind_byte = [0u8; 1];
    reader.read_exact(&mut field_kind_byte)?;
    ...
    for _ in 0..count {
        constraints.push(deserialize_constraint_node(reader)?);
    }
...
fn deserialize_entity_node<R: Read>(reader: &mut R) -> IoResult<EntityNode> {
    ...
    if field_count != 4 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for Entity node",
        ));
    }
...
fn deserialize_attribute_node<R: Read>(reader: &mut R) -> IoResult<AttributeNode> {
    ...
    if field_count < 2 || field_count > 3 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for Attribute node",
        ));
    }
...
fn deserialize_rule_ref_node<R: Read>(reader: &mut R) -> IoResult<RuleRefNode> {
    ...
    if kind != 17 {
        // NodeKind::RuleRef as u16
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Expected RuleRef node",
        ));
    }
    ...
    if field_count != 1 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for RuleRef node",
        ));
    }
...
fn deserialize_constraint_ref_node<R: Read>(reader: &mut R) -> IoResult<ConstraintRefNode> {
    ...
    if kind != 18 {
        // NodeKind::ConstraintRef as u16
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Expected ConstraintRef node",
        ));
    }
    ...
    if field_count != 1 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for ConstraintRef node",
        ));
    }
...
fn deserialize_rule_node<R: Read>(reader: &mut R) -> IoResult<RuleNode> {
    ...
    if field_count != 4 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for Rule node",
        ));
    }
...
fn deserialize_parameter_node<R: Read>(reader: &mut R) -> IoResult<ParameterNode> {
    ...
    if field_count != 2 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for Parameter node",
        ));
    }
...
fn deserialize_flow_node<R: Read>(reader: &mut R) -> IoResult<FlowNode> {
    ...
    if field_count != 2 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for Flow node",
        ));
    }
...
fn deserialize_constraint_node<R: Read>(reader: &mut R) -> IoResult<ConstraintNode> {
    ...
    if field_count != 3 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for Constraint node",
        ));
    }
...
fn deserialize_binary_expr_node<R: Read>(reader: &mut R) -> IoResult<BinaryExprNode> {
    ...
    if field_count != 3 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for BinaryExpr node",
        ));
    }
...
fn deserialize_unary_expr_node<R: Read>(reader: &mut R) -> IoResult<UnaryExprNode> {
    ...
    if field_count != 2 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for UnaryExpr node",
        ));
    }
...
fn deserialize_literal_expr_node<R: Read>(reader: &mut R) -> IoResult<LiteralExprNode> {
    ...
    if field_count != 1 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for LiteralExpr node",
        ));
    }
...
fn deserialize_identifier_expr_node<R: Read>(reader: &mut R) -> IoResult<IdentifierExprNode> {
    ...
    if field_count != 1 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for IdentifierExpr node",
        ));
    }
...
fn deserialize_call_expr_node<R: Read>(reader: &mut R) -> IoResult<CallExprNode> {
    ...
    if field_count != 2 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for CallExpr node",
        ));
    }
...
fn deserialize_assign_action_node<R: Read>(reader: &mut R) -> IoResult<AssignActionNode> {
    ...
    if field_count != 2 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for AssignAction node",
        ));
    }
...
fn deserialize_emit_action_node<R: Read>(reader: &mut R) -> IoResult<EmitActionNode> {
    ...
    if field_count != 1 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for EmitAction node",
        ));
    }
...
fn deserialize_flow_step_node<R: Read>(reader: &mut R) -> IoResult<FlowStepNode> {
    ...
    if field_count < 2 || field_count > 3 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for FlowStep node",
        ));
    }
...
fn deserialize_type_node<R: Read>(reader: &mut R) -> IoResult<TypeNode> {
    ...
    if field_count != 2 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for Type node",
        ));
    }
...
fn deserialize_identifier_node<R: Read>(reader: &mut R) -> IoResult<IdentifierNode> {
    ...
    if field_count != 1 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Invalid field count for Identifier node",
        ));
    }
...
fn deserialize_binary_operator<R: Read>(reader: &mut R) -> IoResult<BinaryOperator> {
    ...
    if field_kind_byte[0] != 5 {
        // FieldKind::Enum as u8
...
fn deserialize_literal_value<R: Read>(reader: &mut R) -> IoResult<LiteralValue> {
    ...
    match value_type_byte[0] {
        ...
        1 => {
            ...
            Ok(LiteralValue::Float(f64::from_bits(u64::from_le_bytes(
                float_bytes,
            ))))
        }
        2 => {
...
fn deserialize_string<R: Read>(reader: &mut R) -> IoResult<String> {
    ...
    if field_kind_byte[0] != 2 {
        // FieldKind::StringId as u8
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "Expected StringId field",
        ));
    }
