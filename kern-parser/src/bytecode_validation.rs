...
impl std::fmt::Display for BytecodeValidationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BytecodeValidationError::InvalidInstruction(msg) => write!(f, "Invalid instruction: {}", msg),
            ...
            BytecodeValidationError::UndefinedSymbol(symbol) => write!(f, "Undefined symbol: {}", symbol),
            BytecodeValidationError::InvalidJumpTarget(target) => write!(f, "Invalid jump target: {}", target),
            ...
            BytecodeValidationError::UnsupportedFeature(feature) => write!(f, "Unsupported feature: {}", feature),
        }
    }
}
...
impl BytecodeCompatibilityValidator {
    pub fn new() -> Self {
        ...
        instruction_operands.insert("JMP_IF".to_string(), vec!["flag_register".to_string(), "instruction_index".to_string()]);
        instruction_operands.insert("HALT".to_string(), vec![]);
        instruction_operands.insert("LOAD_SYM".to_string(), vec!["dest_register".to_string(), "symbol_table_index".to_string()]);
        instruction_operands.insert("LOAD_NUM".to_string(), vec!["dest_register".to_string(), "immediate_value".to_string()]);
        instruction_operands.insert("MOVE".to_string(), vec!["dest".to_string(), "src".to_string()]);
        instruction_operands.insert("COMPARE".to_string(), vec!["reg_a".to_string(), "reg_b".to_string(), "comparator_enum".to_string()]);
    ...
    pub fn validate_program(&mut self, program: &Program) -> Result<(), Vec<BytecodeValidationError>> {
        ...
        for def in &program.definitions {
            match def {
                Definition::Entity(entity_def) => {
                    self.symbol_types.insert(entity_def.name.clone(), "entity".to_string());
                ...
                Definition::Rule(rule_def) => {
                    self.symbol_types.insert(rule_def.name.clone(), "rule".to_string());
                },
                Definition::Flow(flow_def) => {
                    self.symbol_types.insert(flow_def.name.clone(), "flow".to_string());
                },
                Definition::Constraint(constraint_def) => {
                    self.symbol_types.insert(constraint_def.name.clone(), "constraint".to_string());
                },
            }
        }
    }
    ...
    fn validate_entity_def(&mut self, entity_def: &EntityDef) {
        ...
        for field in &entity_def.fields {
            if field_names.contains(&field.name) {
                self.errors.push(BytecodeValidationError::InvalidOperand(
                    format!("Duplicate field name '{}' in entity '{}'", field.name, entity_def.name)
                ));
            } else {
        ...
        match expression {
            ...
            Expression::Predicate(predicate) => {
                ...
                if !self.symbol_types.contains_key(&predicate.name) {
                    self.errors.push(BytecodeValidationError::UndefinedSymbol(predicate.name.clone()));
                }
        ...
        match term {
            Term::Identifier(name) => {
                ...
                if !self.symbol_types.contains_key(name) {
                    self.errors.push(BytecodeValidationError::UndefinedSymbol(name.clone()));
                }
            },
            Term::Number(_) => {
            ...
            Term::QualifiedRef(entity_name, field_name) => {
                ...
                if !self.entity_fields.contains_key(entity_name) {
                    self.errors.push(BytecodeValidationError::UndefinedSymbol(entity_name.clone()));
                    return;
                }
                ...
                if let Some(fields) = self.entity_fields.get(entity_name) {
                    if !fields.contains_key(field_name) {
                        self.errors.push(BytecodeValidationError::InvalidOperand(
                            format!("Field '{}' does not exist in entity '{}'", field_name, entity_name)
                        ));
                    }
                }
            }
...
mod tests {
    ...
    fn test_bytecode_validator_with_error() {
        ...
        assert!(matches!(errors[0], BytecodeValidationError::UndefinedSymbol(_)));
    }
}