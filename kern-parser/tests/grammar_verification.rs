...
// Test the complete grammar structure with nested elements
#[test]
fn test_complete_grammar_structure() {
    let input = r#"
        entity Farmer {
            id
            name
        ...
        rule ValidateFarmer:
            ...
            then validate_farmer(farmer), log_validation(farmer.id)

        rule ProcessCrop:
            if crop.type != "" and crop.season == "spring"
            then process_crop(crop), update_yield(crop, 100)

        flow FarmManagement {
            load_farmers(),
            validate_farmers(),
            if has_crops() then process_crops() else report_no_crops(),
            generate_reports(),
            halt
        }

        constraint ValidFarmerId: farmer.id > 0
        constraint ValidCropType: crop.type != ""
    "#;
