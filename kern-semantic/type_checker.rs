...
impl TypeChecker {
                ...
                match op {
                    Comparator::Equal | Comparator::NotEqual => {
                        ...
                        if left_type != right_type {
                            let location =
                                crate::symbol::SourceLocation::new("unknown".to_string(), 0, 0); // In real implementation, get from AST
                            self.errors.push(
                                TypeError::TypeMismatch {
                    ...
                    | Comparator::LessEqual => {
                        ...
                        if !TypeCheckerUtil::validate_numeric(&left_type)
                            || !TypeCheckerUtil::validate_numeric(&right_type)
                        {
                            let location =
                                crate::symbol::SourceLocation::new("unknown".to_string(), 0, 0); // In real implementation, get from AST
                            self.errors.push(
    ...
    fn check_predicate(&mut self, predicate: &Predicate) -> TypeDescriptor {
            ...
            .resolve_symbol(&predicate.name)
        {
            ...
            let _location = crate::symbol::SourceLocation::new("unknown".to_string(), 0, 0); // In real implementation, get from AST
            self.errors
